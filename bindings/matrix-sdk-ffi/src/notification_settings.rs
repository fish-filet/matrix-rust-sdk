...
impl NotificationSettings {
    pub(crate) fn new(
        ...
        Self { sdk_client, sdk_notification_settings, event_handler: Arc::new(RwLock::new(None)) }
    }
}

impl Drop for NotificationSettings {
    fn drop(&mut self) {
        println!("[NotificationSettings] drop()");
        RUNTIME.block_on(async move {
            if let Some(event_handler) = self.event_handler.read().await.as_ref() {
                println!("[NotificationSettings] remove event handler");
    ...
    pub fn set_delegate(&self, delegate: Option<Box<dyn NotificationSettingsDelegate>>) {
        if let Some(delegate) = delegate {
            ...
            let handler = move |_: PushRulesEvent| {
                ...
                async move {
                    println!("[NotificationSettings] PushRulesEvent received.");
                    delegate.notification_settings_did_change();
                }
            };
            println!("[NotificationSettings] set event handler");
            // Add the event handler
            RUNTIME.block_on(async move {
                let event_handler = self.sdk_client.add_event_handler(handler);
                *self.event_handler.write().await = Some(event_handler);
